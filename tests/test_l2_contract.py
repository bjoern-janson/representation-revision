from representation_revision.assay import FOLLOW_UP_PROBE_POOL, FUTURE_INPUTS, TRIGGER
...
def test_trigger_discrepancy_is_causal_input_to_diagnosis():
    result = rra(TRIGGER, (), G0, (G0, G1, ), FOLLOW_UP_PROBE_POOL)
    assert target_is_zero(TRIGGER)
...
def test_non_failure_retains_current_generator():
    result = rra((0, 0, 0), (), G1, (G0, G1), FOLLOW_UP_PROBE_POOL)
    assert result.generator_id == G1.generator_id
...
def test_paired_shadow_is_genuinely_held_out_and_identical():
    ...
    assert set(FUTURE_INPUTS).isdisjoint({TRIGGER, *FOLLOW_UP_PROBE_POOL})
