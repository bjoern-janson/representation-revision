...
fixed certified universe
  ...
  -> G*-specific held-out consequence
```

The four causal burdens are:

- **W — What changed?** `G0 != G*` at the generator interface.
- **C — What caused the change?** corrective evidence preceded and causally discriminated the selected candidate, without selected-candidate identity leakage.
- **P — Did it persist?** the selected `G*` remains the generator used throughout the post-adoption held-out set.
- **E — What did persistence cause?** the held-out consequence is specific to `G*` and survives shadow and compensation controls.
...
`representation_revision.assay.run_scientific_assay()` is retained as historical evidence. It uses the original three-generator toy `GeneratorSpec` universe and its recorded artifacts. The certificate is deliberately not retrofitted into that historical execution, because doing so would silently change the experimental object.
...
The **compensation** control is a separate future condition permitted to reproduce useful behavior while retaining `G0`. It tests the residual alternative that apparent L2 success is downstream behavioral rescue rather than representation revision.

## Claim boundary

The operational experiment targets evidence-driven persistent revision of the representation generator. It makes no L3 claim and no claim that self-reference, recursion, consciousness, or general intelligence is necessary for the observed effect.
